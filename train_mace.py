...
# from gnn.models import CrystGraphConv as LatticeGNN
from gnn.callbacks import PrintTableMetrics, upload_evaluations, local_plot_evaluations
...
def main() -> None:
    ...
    # wandb_logger.experiment.config["desc"]  = f"Overfit, weight decay 1e-8, L2"
    ...
    local_plot_evaluations(results, './figures/train_mace_cartesian.png')
    val_results = trainer.predict(model=model, dataloaders=val_loader, return_predictions=True, ckpt_path='best')
    local_plot_evaluations(val_results, './figures/valid_mace_cartesian.png')
